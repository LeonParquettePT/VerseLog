...
- **[Download verselog-linux (Linux)](https://github.com/LeonParquettePT/VerseLog/releases/tag/v0.1.0-linux)** — built via a disposable GitHub Actions runner, verified in CI (builds and runs correctly headlessly). Run `chmod +x verselog-linux` after downloading. Manual verification that the graphical window renders on a real Linux desktop is still pending — see the Release notes.
