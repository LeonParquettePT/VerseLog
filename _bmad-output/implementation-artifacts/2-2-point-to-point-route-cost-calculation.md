...
- 2026-07-09: Story 2.3's code review found `LoadingPlanCalculator` had to fetch the same ship a second time from `ShipReferenceStore` just to read `cargo_capacity_scu`, since `calculate()` fetched it internally but never exposed it. Added a `ship: ShipReference` field to `RouteCost` so callers that already have a `RouteCost` never need a redundant ship lookup. Purely additive (no existing field changed); 60/60 tests passing.
