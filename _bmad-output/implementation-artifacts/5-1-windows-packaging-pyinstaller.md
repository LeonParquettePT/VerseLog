...
- **Known distribution issue confirmed by the user: Windows blocks the unsigned exe on first run** (`Une stratégie de contrôle d'application a bloqué ce fichier`). This is a common false positive for unsigned PyInstaller executables, not a real security problem — the file is not deleted, just blocked from executing.
- **Correction (2026-07-10):** the user's follow-up confirmed the actual blocker is **Windows 11 Smart App Control**, not (only) Defender SmartScreen — these are different mechanisms with different remedies, confirmed via web research:
  - **Smart App Control** ("we couldn't verify its publisher") has **no per-app bypass at all** — no "Run anyway", no exception, no allowlist. The only remedy is to temporarily disable Smart App Control in Windows Security → App & Browser Control, run the app, then re-enable it. Prior to an early-2026 Windows update this toggle required a full Windows reinstall to reverse; that restriction has since been lifted, so it's now freely reversible.
  - **SmartScreen** (the older, milder mechanism — "an app control policy has blocked this file") *does* offer "More info → Run anyway". My initial release notes/docs wording assumed this was the only mechanism in play and gave SmartScreen-only guidance; corrected in the Release notes, `docs/index.html`, and both READMEs to describe both mechanisms accurately, since giving wrong instructions here would leave real users stuck.
  - Real long-term fix (not implemented, tracked as a future improvement): code-signing the exe (e.g. a free cert for open-source projects via SignPath.io) so Windows can verify the publisher and neither mechanism triggers.
- **Second correction, same conversation:** Smart App Control re-evaluates the file on every launch, not just once — confirmed via web research that re-enabling it after a successful run will likely re-block the (still-unsigned) exe on the next launch. Updated guidance everywhere to say: keep Smart App Control disabled while actively using VerseLog, or toggle it off before each launch — not "run once then re-enable," which would have left users re-blocked and confused.
