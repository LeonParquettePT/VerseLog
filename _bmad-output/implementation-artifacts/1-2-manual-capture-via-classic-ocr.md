...
- 2026-07-09: Manual real-image verification (two real screenshots supplied by the user, run through actual `pytesseract`) found and fixed two real-world OCR bugs, not caught by the synthetic-text fixtures: (1) Tesseract does not reliably render the game's `¤` currency glyph — dropped entirely in one screenshot, misread as `&` in the other — so `_REWARD_RE` was changed from a literal `¤\s*([\d,]+)` match to `Reward\D*?([\d,]+)`, anchoring on the word instead of the glyph; (2) a location name that wraps across two lines in the UI came through OCR with an embedded newline, which would have broken exact-name lookups against `LocationReferenceStore` (Story 2.2) — fixed by adding a `_clean_location_name()` helper that collapses whitespace/newlines before building the `Contract`. Both fixes verified against the real Hauling-contract screenshot end-to-end (`Contract(departure='Port Tressler', arrival='Greycat Stenton IV Production Complex-A', scu=6, reward=50250.0, remaining_time=None)`); a second, Mercenary-category screenshot correctly raises `ContractParseError` since it doesn't follow the Hauling Primary-Objectives shape (expected — matches the known limitation already stated in Dev Notes). Added regression tests for both fixes. 55/55 tests passing.
