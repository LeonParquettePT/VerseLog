...
- `uv run --extra dev pytest -q` → `187 passed` after the code-review fixes below (183 before review + 4 net new/changed tests). Known pre-existing Tk() flake observed a couple of times during this story, always on unrelated files, cleared on immediate retry — same documented environment issue, not caused by this story's changes.
...
- Second rebuild (after Task 5, before code review): `dist/verselog-installer.exe` → 11,761,993 bytes (11.76 MB). Confirmed via `grep -ac` string search that `pytesseract`/`ollama`/`pydantic`/`httpx`/`PIL` all dropped to 0 occurrences (previously non-zero, `PIL` had been 89).
- Third rebuild (after code review's `requests`-for-consistency fix): `dist/verselog-installer.exe` → 15,110,317 bytes (15.1 MB) — `requests` (already a project dependency, used elsewhere for HTTP) adds back some weight versus raw `urllib.request`, but nowhere near the `ollama` client's own `httpx`/`anyio`/`pydantic` chain. Net result: 27.8 MB → 15.1 MB, a 46% reduction, using the codebase's own established HTTP convention instead of a one-off `urllib` implementation.
- Verified the real Ollama REST API's JSON response shape directly: queried `http://localhost:11434/api/tags` on this dev machine's own running Ollama instance, confirmed each model entry has both `"name"` and `"model"` keys with identical values.
- Verified `pytesseract.get_tesseract_version()`'s own real source (via `inspect.getsource`) to confirm the rewritten subprocess-based check replicates the same underlying mechanism before rewriting it.
- Code review (high effort, 5 finder passes) found 6 real issues, all fixed: (1) `model["model"]` KeyError risk on an Ollama response missing that key → `.get()` with an `isinstance`/truthiness guard; (2) unguarded `data.get("models", [])` could crash on a malformed/non-standard response shape → defensive `models = data.get("models") or []`, regression test added for `{"models": null}`; (3) `OLLAMA_HOST` set without a scheme (a commonly-documented form, e.g. `"0.0.0.0:11434"`) would raise uncaught → `_default_ollama_base_url()` now normalizes it, 2 regression tests added; (4) hand-rolled `urllib.request` + module-level monkeypatching was inconsistent with this codebase's own established convention (`CommunityAPIProvider`'s injectable `http_get=requests.get`) → rewritten to match, using `requests` (already a project dependency); (5) `subprocess.run(["tesseract", "--version"])` had no timeout, unlike the sibling HTTP call → added `timeout=_TESSERACT_TIMEOUT_SECONDS`, regression test for a hang; (6) `recommend_tier`'s `cpu_count` parameter was accepted but never used in the threshold → removed entirely rather than kept as a confusing dead parameter. A 7th finding (altitude): `DEFAULT_VISION_MODEL` had been placed in `verselog/core/`, but a bare Ollama model tag isn't a domain concept — moved to `verselog/adapters/capture/vision_model.py` instead, still avoiding `vision_provider.py`'s heavy transitive imports since it's its own dependency-free file.
...
- Fixed the actual file-size cause, discovered mid-story: `PrerequisiteChecker` (Story 5.3, used by `ComponentSelectionStep`) independently imported `pytesseract`/`ollama` for its own legitimate "is it installed" check, pulling in the same heavy dependency chain (`Pillow`, `pydantic`, `httpx`, `anyio`) regardless of the benchmark step's own fix. Rewrote it to use a direct `subprocess` call and (after code review) `requests.get` instead — same behavior, verified against this machine's real, running Tesseract/Ollama installs, matching the codebase's own established HTTP convention.
- `verselog-installer.exe`: 27.8 MB → 15.1 MB (46% smaller). `verselog.exe` itself is unaffected — it still needs the full capture stack for its own real OCR/vision work.
- Code review (high effort) found and fixed 7 real issues — see Debug Log for the full list. 187/187 tests passing.
...
- `src/verselog/adapters/capture/vision_model.py` (new — `DEFAULT_VISION_MODEL`, moved out of `vision_provider.py`; placed in `adapters/capture/`, not `core/`, per code-review altitude finding)
- `src/verselog/adapters/capture/vision_provider.py` (modified — imports the constant instead of defining it)
- `src/verselog/adapters/system/prerequisite_checker.py` (modified — subprocess + `requests.get` instead of pytesseract/ollama client libraries)
